...
2. **Then apply these spacing principles** to decide _which_ token belongs _where_ — inner-group vs between-group, vertical vs horizontal, tight vs loose — when the brief or component spec is silent.
...
Apply the same logic to search bars, filter rows, and settings panels. See also `ui-principles.md` §0.1 — _Icon inset on inputs must balance both sides_.
...
| **`spacing-principles.md` (this file)**            | How to _choose_ and _layer_ spacing tiers for grouping and hierarchy              |
